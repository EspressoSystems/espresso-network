...
impl<TYPES: NodeType, I: NodeImplementation<TYPES>, V: Versions> SystemContext<TYPES, I, V> {
    ...
    pub async fn run_tasks(&self) -> SystemContextHandle<TYPES, I, V> {
        ...
        let (block_ready_sender, _) = tokio::sync::broadcast::channel(1000);
    ...
    V: Versions,
{
    ...
    async fn spawn_twin_handles(
        ...
        // create each handle
        let (left_block_ready_sender, _) = tokio::sync::broadcast::channel(1000);
        let mut left_handle = SystemContextHandle::<_, I, _> {
            ...
            epoch_height,
            block_ready_sender: left_block_ready_sender,
        };

        let (right_block_ready_sender, _) = tokio::sync::broadcast::channel(1000);
        let mut right_handle = SystemContextHandle::<_, I, _> {
            ...
            epoch_height,
            block_ready_sender: right_block_ready_sender,
        };
