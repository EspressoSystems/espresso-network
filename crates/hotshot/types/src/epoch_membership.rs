...
use crate::{
    ...
    traits::{
        ...
        storage::StorageStoreDrbResultFn,
    },
...
pub struct EpochMembershipCoordinator<TYPES: NodeType> {
    ...
    storage_store_drb_result_fn: Option<StorageStoreDrbResultFn<TYPES>>,
...
impl<TYPES: NodeType> Clone for EpochMembershipCoordinator<TYPES> {
    fn clone(&self) -> Self {
        Self {
            ...
            storage_store_drb_result_fn: self.storage_store_drb_result_fn.clone(),
    ...
    Self: Send,
{
    ...
    async fn catchup(self, epoch: TYPES::Epoch) -> Result<EpochMembership<TYPES>> {
        ...
        if let Some(cb) = &self.storage_store_drb_result_fn {
