...
pub async fn verify_leaf_chain<T: NodeType>(
    ...
    // carried in its vote data: the chain deciding an epoch-boundary leaf spans
    // the transition, so its QCs are signed by different committees.
    let check_qc = |qc: QuorumCertificate2<T>| -> anyhow::Result<()> {
        ...
        let stake_table_entries = StakeTableEntries::from_iter(membership.stake_table()).0;
        qc.is_valid_cert(
            &stake_table_entries,
