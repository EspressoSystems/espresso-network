...
pub(crate) struct ValidatedStateManager<TYPES: NodeType> {
    ...
    in_progress_requests: HashMap<Commitment<Leaf2<TYPES>>, StateRequest<TYPES>>,
    in_progress_headers: HashSet<ViewNumber>,
    pending_requests: BTreeMap<Commitment<Leaf2<TYPES>>, Vec<StateEvent<TYPES>>>,
    tasks: JoinSet<CompletedRequest<TYPES>>,
    instance_state: Arc<TYPES::InstanceState>,
}

impl<TYPES: NodeType> ValidatedStateManager<TYPES> {
    pub(crate) fn new(instance_state: Arc<TYPES::InstanceState>) -> Self {
        Self {
    ...
    /// Poll for the next completed result from spawned validation/header tasks.
    pub(crate) async fn next_result(&mut self) -> Option<ConsensusInput<TYPES>> {
        let completed = self.tasks.join_next().await?.ok()?;
        match completed {
            CompletedRequest::State(result) => self.handle_state_completed(result),
            CompletedRequest::Header(result) => self.handle_header_completed(result),
        }
    }

    pub(crate) fn handle_event(&mut self, event: StateEvent<TYPES>) {
        match event {
    ...
    fn handle_request_state(&mut self, request: StateRequest<TYPES>) {
    ...
    fn handle_request_header(&mut self, request: HeaderRequest<TYPES>) {
    ...
    fn start_pending(&mut self, finished_commitment: Commitment<Leaf2<TYPES>>) {
...
mod test {
    ...
    impl StateTestHarness {
        ...
        fn request_state(&mut self, request: StateRequest<TestTypes>) {
            self.manager.handle_request_state(request);
        }

        fn request_header(&mut self, request: HeaderRequest<TestTypes>) {
            self.manager.handle_request_header(request);
        }

        /// Wait for spawned tasks to complete and collect all results.
        async fn collect_results(&mut self) -> Vec<ConsensusInput<TestTypes>> {
            // Give spawned tasks time to finish.
            tokio::time::sleep(std::time::Duration::from_millis(100)).await;
            let mut results = Vec::new();
            while let Some(Ok(completed)) = self.manager.tasks.try_join_next() {
                let input = match completed {
                    CompletedRequest::State(result) => self.manager.handle_state_completed(result),
                    CompletedRequest::Header(result) => {
                        self.manager.handle_header_completed(result)
                    },
                };
    ...
    async fn test_state_request_missing_parent_inserts_empty() {
        ...
        harness.request_state(make_state_request(&test_data.views[0]));
        let results = harness.collect_results().await;

        // No StateVerified because parent was missing — empty state inserted.
        assert_eq!(
    ...
    async fn test_sequential_state_requests() {
        ...
        harness.request_state(make_state_request(&test_data.views[0]));
        harness.collect_results().await;

        // Request view 2 — parent (view 1) should now exist.
        harness.request_state(make_state_request(&test_data.views[1]));
        let results = harness.collect_results().await;
    ...
    async fn test_header_request_with_parent() {
        ...
        harness.request_header(header_req);
        let results = harness.collect_results().await;
