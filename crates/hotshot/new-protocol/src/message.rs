...
pub enum ConsensusMessage<T: NodeType, S> {
    ...
    /// A node's reassembled VID share, broadcast (decoupled from Vote1) so
    /// every node can reconstruct the block.
    VidShareBroadcast(VidDisperseShare2<T>),
    /// A fragment of a node's VID share, unicast by the leader; the recipient
    /// reassembles the fragments into a complete share.
