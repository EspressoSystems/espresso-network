...
impl<T: NodeType> VidReconstructor<T> {
    ...
    /// Register a leader-event tracer. Production builds leave this `None`.
    /// The bench wires this in `node.rs` so `RecoverVMinus1Start` /
    /// `RecoverVMinus1DecodeEnd` / `RecoverVMinus1End` get emitted around the
    /// `spawn_blocking` reconstruction (see `reconstruct()` below).
    pub fn set_tracer(&mut self, tracer: Option<crate::leader_trace::LeaderTracerHandle>) {
        self.tracer = tracer;
    }
