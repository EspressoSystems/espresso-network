    ...
    S: NewProtocolStorage<T>,
{
    ...
    pub fn process_consensus_output(
        ...
        match output {
            ...
            // `ConsensusOutput::SendVidShares` arm dropped: main PR #4517 moved
            // share sending into the disperser, so consensus no longer emits
            // this output.  The parallel fan-out + VidSharesUnicastStart/End
            // trace events from HEAD now belong inside the disperser; re-add
            // them there if/when measurement granularity is needed.
