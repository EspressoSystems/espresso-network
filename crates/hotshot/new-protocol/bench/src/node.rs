...
fn build_test_block(
    ...
    // be split, but it does not depend on the erasure code either, so it rides
    // alongside it rather than extending the leader's critical path. It has to
    // finish before the header, so the dispersal is the only thing it can hide
    // behind.
