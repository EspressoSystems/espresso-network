...
use hotshot_types::{
    ...
    traits::{
        election::Membership,
        node_implementation::{ConsensusTime, NodeType},storage::storage_store_drb_result
    },
};
...
async fn test_network_task() {
    ...
    let coordinator = EpochMembershipCoordinator::new(membership, Some(storage_store_drb_result(storage.clone())),config.epoch_height);
