...
/// Peers and timeouts for state catchup over HTTP.
#[derive(Clone, Debug)]
...
/// A network config obtained without the orchestrator.
pub(crate) enum LoadedNetworkConfig {
    /// Saved by a previous run of this node; already persisted.
    Persisted(NetworkConfig<SeqTypes>),
    /// Fetched from a running peer; the caller persists it once genesis overrides are applied.
    Fetched(NetworkConfig<SeqTypes>),
}

/// The state machinery every node needs, whether or not it takes part in consensus: the L1
/// client and catchup providers (inside `node_state`), the stake table coordinator, and the
/// persistence they share.
    ...
    Arc<P>: Storage<SeqTypes>,
{
    ...
    .await?;
    let (mut network_config, wait_for_orchestrator, persist_config) = match loaded_network_config {
        Some(LoadedNetworkConfig::Persisted(config)) => (config, false, false),
        Some(LoadedNetworkConfig::Fetched(config)) => (config, false, true),
...
/// Load the network config saved by a previous run, or fetch it from `config_peers`.
///
/// Returns `None` when neither is available: a fresh network bootstraps from the orchestrator.
...
/// Apply the consensus parameters only genesis knows.
///
/// The orchestrator and peer-fetched configs do not carry epoch, DRB or stake table capacity
/// settings; persisting a config before this runs would store zeros for them.
...
/// Build the state machinery shared by every node type.
///
/// Connects the L1 client, sets up state catchup, starts the stake table fetcher, loads the
/// membership and its coordinator, and assembles the [`NodeState`] that evaluates the state
/// transition. Nothing here depends on taking part in consensus.
///
...
mod test {
    ...
    /// Every node persists its network config after this runs, so the values only genesis knows
    /// have to land in it, with the stake table capacity defaulting when genesis leaves it unset.
    #[test]
