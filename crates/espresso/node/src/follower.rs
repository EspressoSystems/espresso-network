...
//! consensus.
//!
//! The follower fetches every decided block from upstream query nodes and verifies it with the
//! [`light_client`](::light_client) crate: the finality proof against the stake table of the
//! block's epoch, payload and VID common against the header, and the cert2 signature. It then
//! hands the block to the query API through the same sink a validator hands its decides to, so
//! every API module serves it unchanged. Stake tables come from L1 through the coordinator a
//! validator uses, driven by the epoch roots the follower decides.
//!
...
/// How a follower polls its upstream query nodes.
#[derive(Clone, Debug)]
pub struct FollowerOptions {
    /// How long to wait between polls for new blocks.
    pub poll_interval: Duration,
    /// The most blocks one poll ingests; older heights are left to the query service's backfill.
    pub max_blocks_per_poll: u64,
    pub light_client: LightClientOptions,
    pub light_client_db: LightClientSqliteOptions,
    /// Root of trust for the light client; derived from the network config when unset.
    pub light_client_genesis: Option<LightClientGenesis>,
}

impl FollowerOptions {
    /// Reject settings under which the follower would poll without ever following.
...
/// Where a follower gets the chain and its configuration from.
#[derive(Clone, Debug)]
pub struct FollowerParams {
    /// Query nodes the follower verifies the chain from.
    pub upstreams: Vec<Url>,
    /// Peers to fetch the network config from; the upstreams when unset.
    pub config_peers: Option<Vec<Url>>,
    pub catchup: CatchupParams,
    /// Per-step timeout for the stake-table catchup walk, at startup and after skipping ahead.
    pub bootstrap_epoch_catchup_timeout: Duration,
    pub options: FollowerOptions,
}

/// A node that follows the chain through the light client.
///
/// Serves the API like a [`SequencerContext`](crate::SequencerContext), without HotShot,
/// cliquenet or validator keys.
...
/// Start a follower: resolve the network config, build the node state and stake table
/// coordinator, and connect the light client. Following begins with
/// [`FollowerContext::start`].
...
impl<P: SequencerPersistence> FollowerContext<P> {
    /// Start following the chain from the height the query database has reached.
    ...
    /// The latest leaf the follower has verified; waits for the first one.
    ...
    /// Stop following and shut down background tasks.
    ...
    /// Wait for the follower's tasks; blocks for as long as the node runs.
...
/// The follower's answers to what the API needs from consensus.
///
...
struct FollowerConsensus {
    ...
    /// The chain config in force at the decided leaf, resolved from its commitment.
    chain_config: Arc<RwLock<ChainConfig>>,
    coordinator: EpochMembershipCoordinator<SeqTypes>,
    /// Shared with the follow loop, which records the upgrades the chain decides.
...
impl ConsensusSource for FollowerConsensus {
    ...
    /// Forward the transaction to the first upstream that accepts it.
...
/// The task that pulls verified blocks from the light client into the query database.
struct Follower<P> {
    light_client: Arc<NodeLightClient>,
    sink: Arc<dyn DecideSink>,
    persistence: Arc<P>,
    ...
    options: FollowerOptions,
    bootstrap_epoch_catchup_timeout: Duration,
    /// The next height to ingest.
    next: u64,
}

/// A block the light client has verified, ready to store.
...
impl<P: SequencerPersistence> Follower<P> {
    ...
    /// Fetch and verify the blocks in `from..to`: one finality proof for the range, the rest
    /// chained to it by parent commitment, and one payload proof request for all of them.
    ...
    /// Store one verified block and act on what it decides.
    ...
    /// Record the upgrade a decided leaf carries, as a validator does on deciding it, so the
    /// API validates state certificates against the version the network runs.
    ...
    /// What a validator's epoch manager does on a decide: derive the stake table two epochs
    /// out at each epoch root, and pass along the DRB the transition block carries.
    ///
    ...
    /// Skipping an epoch root skips the stake table it derives, which later roots build on.
    /// When a skipped root's stake table is missing, walk the membership forward from peers as
    /// the startup catchup does. DRBs the skipped transition blocks carried are left to the
    /// coordinator, which fetches them on demand.
...
/// Seed the first epoch on the membership, as `SystemContext::init` does for a validator.
fn seed_first_epoch(
    coordinator: &EpochMembershipCoordinator<SeqTypes>,
    network_config: &NetworkConfig<SeqTypes>,
) {
...
/// `config_fetched` says whether the network config came from peers on this run, in which case
/// deriving the root of trust from it means trusting them.
