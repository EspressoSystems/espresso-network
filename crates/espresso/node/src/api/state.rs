...
use hotshot_query_service::{
    QueryError,
    availability::{AvailabilityDataSource, BlockId as HsBlockId, LeafId as HsLeafId},
    explorer::{
...
mod tests {
    ...
    // Tripwire: the advertised limits come from `hotshot_query_service`'s `Options` defaults,
    // which `availability::router` now enforces (see its `test_range_limit`). If a dependency
    // change moves them, this fails so the new bound is adopted deliberately rather than
    // silently.
    #[test]
    fn range_limits_track_known_defaults() {
        let options = hotshot_query_service::availability::Options::default();
        assert_eq!(options.small_object_range_limit, 500);
        assert_eq!(options.large_object_range_limit, 100);
