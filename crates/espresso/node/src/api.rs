...
mod test {
    ...
    #[test_log::test(tokio::test(flavor = "multi_thread"))]
    async fn test_epoch_reward_restart_regression() {
        const NUM_NODES: usize = 3;
        const EPOCH_HEIGHT: u64 = 200;
        ...
        while last_height < target_height {
            ...
            if height > last_height {
                ...
                if stalled_polls >= stall_limit {
                    ...
                    panic!(
                        "chain stalled after restart: no new decide for 60s at height \
                         {last_height} (epoch {stalled_epoch}), unable to cross next epoch \
                         boundary at height {next_boundary}. The sparse reward tree could not be \
                         recovered (target height was {target_height})."
                    );
                }
            }
        }
