...
mod test {
    ...
    fn test_reward_proof_endpoint(#[case] upgrade: Upgrade) {
        // Use a custom runtime with 16 MiB worker thread stacks. The test body is large enough
        // that tokio's default 2 MiB stacks overflow during polling.
        tokio::runtime::Builder::new_multi_thread()
            ...
            .block_on(async move {
                ...
                let storage =
                    join_all((0..NUM_NODES).map(|_| SqlDataSource::create_storage())).await;
                let persistence: [_; NUM_NODES] = storage
                    .iter()
                    .map(<SqlDataSource as TestableSequencerDataSource>::persistence_options)
                    .collect::<Vec<_>>()
                    ...
                    if upgrade.base == EPOCH_VERSION {
                        ...
                        for (address, _) in validated_state.reward_merkle_tree_v1.iter() {
                            ...
                            match res.proof.proof {
                                ...
                                other => panic!(
                                    "Expected Present proof for V1 at {height}, addr={address}, \
                                     got {other:?}"
                                ),
                            }
                        }
                        ...
                        wait_until_block_height(&client, "reward-state-v2/block-height", height)
                            .await;
                        ...
                        for (address, _) in validated_state.reward_merkle_tree_v2.iter() {
                            ...
                            match res.proof.proof.clone() {
                                ...
                                other => panic!(
                                    "Expected Present proof for V2 at {height}, addr={address}, \
                                     got {other:?}"
                                ),
                            }
                        ...
                        compare_endpoints(&http, api_port, axum_port, "availability/state-cert/1")
                            .await?;
                        compare_endpoints(
                            &http,
                            api_port,
                            axum_port,
                            "availability/state-cert-v2/1",
                        )
                        .await?;
                        ...
                        compare_endpoints(&http, api_port, axum_port, "availability/limits")
                            .await?;
                        ...
                        compare_ws_endpoints(
                            ...
                            axum_port,
                            &format!(
                                "availability/stream/transactions/{avail_block}/namespace/\
                                 {avail_ns}"
                            ),
                        )
                        .await?;
                        compare_ws_endpoints(
                            ...
                            axum_port,
                            &format!(
                                "availability/stream/blocks/{avail_block}/namespace/{avail_ns}"
                            ),
                        )
                        .await?;
                    }
