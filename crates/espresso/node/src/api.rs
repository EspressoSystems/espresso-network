...
/// Collect VID shares for `block_number` from peers over the request-response protocol until
/// `duration` elapses, keeping only shares that verify against `vid_common_data`.
...
/// The light client's root of trust for a network: its genesis stake table and epoch layout.
...
mod test {
    ...
    /// Bound on each stage of the follower catching up with the validators.
    const FOLLOW_TIMEOUT: Duration = Duration::from_secs(240);

    /// A node without stake follows the chain through the light client and serves it from its
    /// own query API.
    ///
    /// Node 0 is the follower's only upstream: it serves the light-client proofs the follower
    /// verifies, the network config it bootstraps from, catchup for stake tables and state
    /// certificates, and the submit endpoint transactions are forwarded to. The query service
    /// only stores new heights through the follow loop, so the follower's archive reaching the
    /// validators' tip proves it follows live; then the endpoints a staked query node serves are
    /// compared with node 0's.
    #[test_log::test(tokio::test(flavor = "multi_thread"))]
    async fn test_light_client_follower_query_node() -> anyhow::Result<()> {
        ...
        // Transactions submitted to the follower are forwarded to node 0 and sequenced.
        ...
        // The follower serves the network config and light client proofs of its own.
        ...
        // A restart resumes from the stored chain and keeps following. Shut the first follower
        // down before starting the second, so only one node writes to the database.
    ...
    /// A node genesis matching the test network, so the follower evaluates the same chain.
    ///
    /// Mirrors what `TestConfig::init_node` gives its validators: the builder account prefunded
    /// in the genesis state, and an otherwise default genesis header.
    ...
    /// Serve a follower of `params.upstreams` with every API module a query node runs, and
    ...
    /// The follower decides up to `tip` and stores every block it decides.
    ...
    /// Cert2s are only stored at the heights they finalize; find the latest one below `tip`.
    ...
    /// The same chain, down to the cert2 that finalized `height`.
    ...
    /// Stake tables, validators and block rewards come from L1, driven by the epoch roots the
    /// follower decided; state certificates are fetched from node 0 and validated against them.
    ...
    /// Merklized state is rebuilt from the stored chain; catchup serves it from storage.
    ...
    /// Reward claims come from the same rebuilt reward tree.
    ...
    /// What only a validator can answer is not found on the follower.
    ...
    /// Node 0 and the follower answer `path` alike, with node 0 answering at all.
    ///
