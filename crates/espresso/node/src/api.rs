...
mod test {
    ...
    /// A node with no stake follows the chain live through two validators that
    /// pin it as a cliquenet observer peer, and serves what it decides from its
    /// own query API.
    ///
    /// Five validators run the new protocol from genesis. The observer is never
    /// registered in the stake table; nodes 0 and 1 forward it proposals and
    /// certificates, and it uses node 0's query API only for payloads, VID
    /// common and state catchup. The query service only stores new heights from
    /// decides, so the observer's archive reaching the validators' tip proves it
    /// decides from the forwarded feed rather than from HTTP backfill.
    #[test_log::test(tokio::test(flavor = "multi_thread"))]
    async fn test_new_protocol_observer_query_node() -> anyhow::Result<()> {
        ...
        // Node 0 serves the query API the observer backfills payloads from, plus
        // the catchup and light client modules that backfill needs.
        let config = TestNetworkConfigBuilder::<NUM_NODES, _, _>::with_num_nodes()
            .api_config(
                Options::with_port(api_port)
        ...
        // Let the validators run through a few epoch boundaries.
        let mut events = network.server.event_stream();
        wait_for_epochs(&mut events, EPOCH_HEIGHT, EPOCHS_TO_FOLLOW).await;

        // The observer decides up to the validators' tip...
        ...
        // ...and its archive stores them. `node/block-height` counts blocks, so
        // block `tip` is stored once it reports `tip + 1`.
        timeout(
            FOLLOW_TIMEOUT,
        ...
        // A height the new protocol finalized with a cert2 is stored on both
        // nodes; cert2s are only stored at the height they finalize, so scan
        // back from the tip for one.
        ...
        // The forwarded cert2 was stored with the decide.
        ...
        // Payloads are not part of the forwarded feed; they arrive through the
        // HTTP fallback from node 0.
