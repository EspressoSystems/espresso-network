...
/// The node this process runs: a validator, or a follower of the chain through the light client.
...
impl<P: SequencerPersistence> NodeContext<P> {
    /// Start taking part in consensus, or following the chain.
    ...
    /// Wait for the node's tasks; blocks for as long as the node runs.
...
pub async fn main(migrated_envs: Vec<(&str, &str)>) -> anyhow::Result<()> {
    ...
    let (mut telemetry_handle, deferred_warnings, telemetry_init_error) =
        match (opt.key_set.clone().try_into(), telemetry_endpoint.as_ref()) {
            ...
            // A follower runs without the staking key telemetry identifies a node by.
    ...
    S: DataSourceOptions,
{
    ...
    // Start doing consensus, or following the chain.
    ctx.start().await?;
...
/// Start a follower with the API modules requested; `Options::validate_modules` has checked
/// that the http, query and storage-sql modules are among them.
...
/// Add the optional API modules requested on the command line.
