//! Pins the v2 wire encoding to canonical protoJSON (generated by pbjson): lowerCamelCase field
//! names, 64-bit integers as decimal strings, `bytes` as base64, a `oneof` flattened into its
//! parent, and default values omitted. Deserialization accepts both camelCase and the original
//! proto field names.
//!
...
#[test]
fn bytes_serialize_as_padded_base64() {
    let namespace = proto::AvidmGf2Namespace {
        range: Some(proto::ShardRange { start: 0, end: 2 }),
        payload: vec![vec![0x0c, 0xff], vec![]],
        mt_proofs: vec![],
    };
    // v1 writes these shards as arrays of numbers, so a client ported from it has to decode.
    assert_eq!(
        serde_json::to_value(namespace).unwrap(),
        json!({"range": {"end": "2"}, "payload": ["DP8=", ""]})
    );
}

#[test]
fn a_oneof_flattens_into_its_parent_under_the_arm_name() {
    let response = proto::VidShareResponse {
        share: Some(proto::vid_share_response::Share::V2(
            proto::AvidmGf2VidShare { namespaces: vec![] },
        )),
    };
    // The arm name is the only thing that says which VID scheme the share came from.
    assert_eq!(serde_json::to_value(response).unwrap(), json!({"v2": {}}));

    let parsed: proto::VidShareResponse =
        serde_json::from_value(json!({"v0": {"index": 3}})).unwrap();
    let Some(proto::vid_share_response::Share::V0(share)) = parsed.share else {
        panic!("the V0 arm");
    };
