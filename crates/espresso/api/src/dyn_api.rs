...
pub(crate) trait DynHotShotAvailabilityApi: Send + Sync {
    ...
    async fn get_leaf_batch(&self, ranges: Vec<Range<u64>>) -> anyhow::Result<Erased>;
    async fn get_block_batch(&self, ranges: Vec<Range<u64>>) -> anyhow::Result<Erased>;
    async fn get_vid_common_batch(&self, ranges: Vec<Range<u64>>) -> anyhow::Result<Erased>;
...
impl<T: v1::HotShotAvailabilityApi + Send + Sync> DynHotShotAvailabilityApi for T {
    ...
    async fn get_leaf_batch(&self, ranges: Vec<Range<u64>>) -> anyhow::Result<Erased> {
        v1::HotShotAvailabilityApi::get_leaf_batch(self, ranges)
            .await
            .map(erase)
    }
    async fn get_block_batch(&self, ranges: Vec<Range<u64>>) -> anyhow::Result<Erased> {
        v1::HotShotAvailabilityApi::get_block_batch(self, ranges)
            .await
            .map(erase)
    }
    async fn get_vid_common_batch(&self, ranges: Vec<Range<u64>>) -> anyhow::Result<Erased> {
        v1::HotShotAvailabilityApi::get_vid_common_batch(self, ranges)
            .await
            .map(erase)
    }
    async fn get_transaction_by_position(&self, height: u64, index: u64) -> anyhow::Result<Erased> {
...
pub(crate) trait DynLightClientApi: Send + Sync {
    ...
    async fn get_payload_proof_batch(&self, ranges: Vec<Range<u64>>) -> anyhow::Result<Erased>;
...
impl<T: v1::LightClientApi + Send + Sync> DynLightClientApi for T {
    ...
    async fn get_payload_proof_batch(&self, ranges: Vec<Range<u64>>) -> anyhow::Result<Erased> {
        v1::LightClientApi::get_payload_proof_batch(self, ranges)
            .await
            .map(erase)
    }
    async fn get_lc_namespace_proof(&self, height: u64, namespace: u64) -> anyhow::Result<Erased> {
