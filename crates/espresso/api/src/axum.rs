...
pub(crate) fn router_reward<S>(state: S) -> Router
where
...
pub(crate) fn router_availability<S>(state: S) -> Router
where
    S: v1::AvailabilityApi + v1::HotShotAvailabilityApi + Clone + Send + Sync + 'static,
{
    ...
    Router::new()
        ...
        .route(routes::v1::PAYLOAD_BY_HASH_ROUTE, get(get_payload_by_hash))
        .route(
...
pub(crate) fn router_block_state<S>(state: S) -> Router
where
...
pub(crate) fn router_fee_state<S>(state: S) -> Router
where
...
pub(crate) fn router_status<S>(state: S) -> Router
where
    S: v1::StatusApi + Clone + Send + Sync + 'static,
{
    ...
    Router::new()
        .route(
            ...
            get(status_block_height),
        )
        .route(
            routes::v1::STATUS_SUCCESS_RATE_ROUTE,
            get(status_success_rate),
        )
        .route(
...
pub(crate) fn router_config<S>(state: S) -> Router
where
...
pub(crate) fn router_node<S>(state: S) -> Router
where
...
pub(crate) fn router_catchup<S>(state: S) -> Router
where
    S: v1::CatchupApi + Clone + Send + Sync + 'static,
{
    ...
    Router::new()
        ...
        .route(
            ...
            get(catchup_state_cert),
        )
        .with_state(state)
}

pub(crate) fn router_submit<S>(state: S) -> Router
where
    S: v1::SubmitApi + Clone + Send + Sync + 'static,
{
    ...
    Router::new()
        .route(
            ...
            ::axum::routing::post(submit_submit),
        )
        .with_state(state)
}

pub(crate) fn router_state_signature<S>(state: S) -> Router
where
...
pub(crate) fn router_hotshot_events<S>(state: S) -> Router
where
...
pub(crate) fn router_light_client<S>(state: S) -> Router
where
...
pub(crate) fn router_explorer<S>(state: S) -> Router
where
...
pub(crate) fn router_token<S>(state: S) -> Router
where
...
pub(crate) fn router_database<S>(state: S) -> Router
where
