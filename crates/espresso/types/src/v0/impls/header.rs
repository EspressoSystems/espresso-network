...
mod test_headers {
    ...
    async fn test_versioned_header_serialization() {
        ...
        assert_eq!(v8_header.version(), version(0, 8));

        let serialized = serde_json::to_string(&v8_header).unwrap();
        let deserialized: Header = serde_json::from_str(&serialized).unwrap();
        assert_eq!(v8_header, deserialized);

        let v8_bytes = BincodeSerializer::<StaticVersion<0, 8>>::serialize(&v8_header).unwrap();
        let deserialized: Header =
            BincodeSerializer::<StaticVersion<0, 8>>::deserialize(&v8_bytes).unwrap();
        assert_eq!(v8_header, deserialized);
    }
}
