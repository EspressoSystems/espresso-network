...
Matrix over the `demo-ff` (V0.6) genesis file. CI runs 3600s per matrix entry (overridden via `DURATION_SECONDS` in the
workflow env); local default is 300s. Artifact retention: 90 days for samples, 1 day for compose logs.
