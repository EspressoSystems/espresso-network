...
use hotshot_builder_shared::utils::EventServiceStream;
use sequencer::{Genesis, L1Params, SequencerApiVersion};
...
pub async fn main() -> anyhow::Result<()> {
    ...
    let instance_state = build_instance_state(genesis, l1_params, opt.state_peers);
    ...
    start_builder(global_state, opt.sequencer_api_url, builder_server_url).await
}
...
mod test {
    ...
    use espresso_types::{
        ChainConfig, FeeAccount, NamespaceId, Transaction, MOCK_SEQUENCER_VERSIONS,
    };
    ...
    use hotshot_builder_refactored::service::BuilderConfig;
    use hotshot_types::{
        data::Leaf2,
    ...
    /// Builder subscribes to this api, and serves the hotshot client request and the private mempool tx submission
    #[test_log::test(tokio::test(flavor = "multi_thread"))]
    async fn test_builder() {
        let query_port = reserve_tcp_port().expect("No ports free");

        let event_service_url: Url = format!("http://localhost:{query_port}").parse().unwrap();

        let builder_port = reserve_tcp_port().expect("No ports free");
        ...
        let config = BuilderConfig {
            ...
            base_fee: ChainConfig::default().base_fee.as_u64().expect(
            ...
            if let EventType::QuorumProposal { proposal, .. } = event.event {
                ...
                let available_blocks = builder_client
                    .get::<Vec<AvailableBlockInfo<SeqTypes>>>(&format!(
                        "block_info/availableblocks/{parent_commitment}/{next_view_number}/\
