...
pub mod multisig_proposals {
    ...
    pub async fn transfer_ownership_from_multisig_to_timelock(
        ...
        let (proxy_addr, proxy_instance) = match contract {
            ...
            | Contract::StakeTableProxy => {
                let addr = contracts.address(contract).ok_or_else(|| {
                    anyhow!("{contract} (multisig owner) not found, can't upgrade")
                })?;
        ...
        if !params.dry_run && !super::super::is_contract(provider, owner_addr).await? {
            ...
            anyhow::bail!(
                "Proxy owner is not a contract. Expected: {owner_addr:#x}. Use --dry-run to skip \
                 this check."
            );
        }
    ...
    pub fn find_script_path() -> Result<PathBuf> {
        ...
        anyhow::bail!(
            "Upgrade entrypoint script, multisig-upgrade-entrypoint, not found in any of the \
             possible locations"
        );
    }
        ...
        match contracts.address(Contract::StakeTableProxy) {
            ...
            Some(proxy_addr) => {
                ...
                if owner_addr != multisig_address {
                    tracing::error!(
                        "Proxy is not owned by the multisig. Expected: {multisig_address:#x}, \
                         Got: {owner_addr:#x}"
                    );
