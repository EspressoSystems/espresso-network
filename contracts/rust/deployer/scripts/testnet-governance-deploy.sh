...
if is_localhost_rpc "$RPC_URL"; then
    ...
    ESPRESSO_ETH_MULTISIG_PAUSER_ADDRESS="0xa0Ee7A142d267C1f36714E4a8F75612F20a79720"
else
    ...
    ESPRESSO_ETH_MULTISIG_PAUSER_ADDRESS="${ESPRESSO_ETH_MULTISIG_PAUSER_ADDRESS:?ESPRESSO_ETH_MULTISIG_PAUSER_ADDRESS must be set for non-localhost deployments}"
fi
...
[[ -n "${ESPRESSO_PERMISSIONED_PROVER:-}" ]] && \
    BASE_ARGS+=(--permissioned-prover "$ESPRESSO_PERMISSIONED_PROVER")
