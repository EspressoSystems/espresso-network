...
contract BLSSig_Test is Test {
  function genBLSSig(bytes memory message) private returns (BN254.G2Point memory, BN254.G1Point memory) {
    ...
    (BN254.G2Point memory vk, BN254.G1Point memory sig) = abi.decode(result, (BN254.G2Point, BN254.G1Point));
    return (vk, sig);
  }
  ...
  function testFuzz_BLS_hashes_computation(bytes memory input) external {
    ...
    (uint256 fieldElem, BN254.G1Point memory hashToCurveElem) = abi.decode(result, (uint256, BN254.G1Point));
  ...
  function wrapVerifyBlsSig(bytes memory message, BN254.G1Point memory sig, BN254.G2Point memory vk) public view {
    BLSSig.verifyBlsSig(message, sig, vk);
  }
  ...
  function testFuzz_RevertWhen_SignatureIsAnInvalidPoint(uint256 x, uint256 y) external {
    ...
    BN254.G1Point memory invalidPoint = BN254.G1Point(BN254.BaseField.wrap(invalidX), BN254.BaseField.wrap(2));
