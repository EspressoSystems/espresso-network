...
// Target contract
import { PlonkVerifierV4 as V } from "../src/libraries/PlonkVerifierV4.sol";

/// @dev Common helpers/utils for PlonkVerifier tests
contract PlonkVerifierV4CommonTest is Test {
...
contract PlonkVerifierV4_constants_Test is PlonkVerifierV4CommonTest {
...
contract PlonkVerifierV4_verify_Test is PlonkVerifierV4CommonTest {
...
contract PlonkVerifierV4_validateProof_Test is PlonkVerifierV4CommonTest {
...
contract PlonkVerifierV4_computeChallenges_Test is PlonkVerifierV4CommonTest {
        ...
        external
    {
        ...
        bytes memory result = vm.ffi(cmds);
        // diff-test returns 8 fields (alpha, alpha2, alpha3, beta, gamma, zeta, v, u);
        // PlonkVerifierV4 drops alpha3 from the struct, so we decode individually.
        (
            uint256 rustAlpha,
            uint256 rustAlpha2,
            /* alpha3 - unused */,
            ...
            uint256 rustV,
            uint256 rustU
        ) = abi.decode(result, (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256));

        V.Challenges memory c = V._computeChallenges(vk, publicInput, proof);
        assertEq(rustAlpha, c.alpha);
        assertEq(rustAlpha2, c.alpha2);
        assertEq(rustBeta, c.beta);
        assertEq(rustGamma, c.gamma);
        assertEq(rustZeta, c.zeta);
        assertEq(rustV, c.v);
        assertEq(rustU, c.u);
    }
}
