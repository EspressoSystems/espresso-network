    ...
    async fn migrate_types(&self, batch_size: u64) -> anyhow::Result<()> {
        ...
        loop {
            ...
            with_backend!(tx, |inner| {
                ...
                query_builder.push_values(leaf_rows.clone(), |mut b, row| {
                    b.push_bind(row.0)
                        .push_bind(row.1)
                        .push_bind(row.2)
                        .push_bind(row.3)
            ...
            with_backend!(tx, |inner| {
                ...
                query_builder.push_values(vid_rows.clone(), |mut b, row| {
...
mod test {
    ...
    #[apply(sql_backends)]
    async fn test_payload_pruning(#[case] backend: DbBackend) {
        let db = TmpDb::init_for(backend).await;
        let mut storage = SqlStorage::connect(db.config(), StorageConnectionType::Query)
            .await
            .unwrap();
        ...
        {
            let mut tx = storage.read().await.unwrap();
            let (num_payloads,): (i64,) = with_backend!(tx, |inner| {
                sqlx::query_as("SELECT count(*) FROM payload")
                    .fetch_one(inner.as_mut())
                    .await
            })
            .unwrap();
            assert_eq!(num_payloads, 1);
            let (num_vid,): (i64,) = with_backend!(tx, |inner| {
                sqlx::query_as("SELECT count(*) FROM vid_common")
                    .fetch_one(inner.as_mut())
                    .await
            })
            .unwrap();
