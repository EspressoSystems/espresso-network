    ...
    Mode: TransactionMode,
{
    ...
    async fn sync_status_ranges(
        ...
        let range_starts: Vec<(i64,)> = with_backend!(self, |tx| {
            sqlx::query_as(&query)
                .bind(start as i64)
        ...
        let range_ends = if range_starts.len() <= 10 {
            ...
            for (i, &(start,)) in range_starts.iter().enumerate() {
                ...
                let (end,): (i64,) = with_backend!(self, |tx| {
                    sqlx::query_as(&query)
                        .bind(upper_bound)
                        // This query is guaranteed to return one result, since `start` satisfies
                        // the requirements even if nothing else does.
                        .fetch_one(tx.as_mut())
                        .await
                })?;
            ...
            let ends: Vec<(i64,)> = with_backend!(self, |tx| {
                sqlx::query_as(&query)
                    .bind(start as i64)
                    .bind(block_height as i64)
                    .fetch_all(tx.as_mut())
