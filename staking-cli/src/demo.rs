...
pub async fn stake_in_contract_for_test(
        ...
        validator_keys.into_iter().enumerate()
    {
        ...
        if multiple_delegators {
            // Pick a random number of delegators between 2 and 5
            let num_delegators = rng.gen_range(2..=5);
            tracing::info!(
                "validator {val_index}: creating {num_delegators} additional delegators"
            );

            for delegator_index in 0..num_delegators {
                // Generate a random wallet for delegator
                let delegator_wallet: LocalSigner<SigningKey> = SigningKey::random(&mut rng).into();
                let delegator_address = delegator_wallet.address();
                tracing::info!(
                    "validator {val_index}, delegator {delegator_index}: address {delegator_address}"
                );

                // Fund the delegator with ETH
                let tx = TransactionRequest::default()
                ...
                assert!(receipt.status());
                tracing::info!(
                    "validator {val_index}, delegator {delegator_index}: funded with {fund_amount_eth} ETH"
                );

                // Generate a random delegate amount between 100 and 500 ESP
                let random_amount: u64 = rng.gen_range(100..=500);
                let delegate_amount = parse_ether(&random_amount.to_string())?;
                let delegate_amount_esp = format_ether(delegate_amount);
                tracing::info!(
                    "validator {val_index}, delegator {delegator_index}: delegating {delegate_amount_esp} ESP"
                );

                // Transfer ESP tokens to the delegator
                let receipt = token
                ...
                assert!(receipt.status());
                tracing::info!(
                    "validator {val_index}, delegator {delegator_index}: received {fund_amount_esp} ESP"
                );

                // Create provider for this delegator
                let delegator_provider = mk_provider(delegator_wallet.clone());
                ...
                assert!(receipt.status());
                tracing::info!(
                    "validator {val_index}, delegator {delegator_index}: approved {delegate_amount_esp} ESP to stake table"
                );
                ...
                assert!(receipt.status());
                tracing::info!(
                    "validator {val_index}, delegator {delegator_index}: delegation complete"
                );
            }
        }
    }
