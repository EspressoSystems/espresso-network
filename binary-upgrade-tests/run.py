...
# Dropped from the base tag's compose, see drop_builder.
BUILDER_SERVICES = ("permissionless-builder", "fund-builder")
...
def drop_builder(compose: dict) -> dict:
    ...
    #4939 deleted the builder, so the upgrade tag has no image to roll it to.
    Both tags run a 0.6 genesis, which builds blocks in-process, so nothing
    consumes what it builds and the base tag can run without it too.
    """
    services = compose["services"]
    for name in BUILDER_SERVICES:
        services.pop(name)
    for service in services.values():
        for name in BUILDER_SERVICES:
            service.get("depends_on", {}).pop(name, None)
        if env := service.get("environment"):
            # Nodes still require a builder URL they never call, so give them
            # the dead one main uses. Submitters lose theirs and fall back to
            # the node API, which is where main sends them.
            service["environment"] = [
