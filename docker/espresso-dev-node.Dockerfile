...
ENV ESPRESSO_NODE_API_PORT=8770
HEALTHCHECK --interval=1s --timeout=1s --retries=100 CMD curl --fail http://localhost:${ESPRESSO_NODE_API_PORT}/status/block-height || exit 1
...
ENV ESPRESSO_NODE_STORAGE_PATH=/data/espresso
