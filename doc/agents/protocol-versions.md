...
- V0_7, `TIMEOUT_EPOCH_VERSION` (also `NO_BUILDER_COMMITMENT_VERSION` and `MAX_SUPPORTED_VERSION`): timeout
  certificates bind the epoch they were collected
  ...
  the network need not restart. `Header::V7` is the V0_6 header without `builder_commitment`. That field was a SHA-256
  over the whole payload that every leader computed and no validator checked: the fee signature covers the fee amount,
  the metadata and the VID commitment, and nothing recomputed the value from the payload. From this version the leader
  skips the hash (`BlockBuilder::request_block`), `Header::builder_commitment` returns `None`, the `BlockHeader` trait
  impl reports the digest of no bytes, and the v2 API renders the header with the `HeaderV7` shape. One consequence to
  expect in operation: a
