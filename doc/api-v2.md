...
merges `rest::reward_service_rest_router`. So that a generated client cannot ship methods that always fail, those
operations are marked `deprecated` with a summary saying they are not implemented; the list lives in
`UNIMPLEMENTED_SERVICES` in `generated/openapi.rs` and has to shrink when the service is wired up.
