            ...
            let upgrade = match version {
                version if version >= EpochVersion::VERSION => {
                    ...
                    let args = DeployerArgsBuilder::default()
                        ...
                        .timelock_admin(self.signer.address())
                        .timelock_proposers(vec![self.signer.address()])
                        .timelock_executors(vec![self.signer.address()])
                        .token_timelock_delay(U256::from(10))
                        .token_timelock_admin(self.signer.address())
                        .token_timelock_proposers(vec![self.signer.address()])
                        .token_timelock_executors(vec![self.signer.address()])
                        .build()
                        .unwrap();
